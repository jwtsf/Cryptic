...
{
    FILE *fpt;
    fpt = fopen("Results/WolfsslRSA-3072-OAEP2.csv", "w+");
    ...
    for (int L = 11; L < (KEY_LENGTH/8) - (2*HASH_LENGTH/8) -1 ; L=L+10)
    {
        ...
        float time_spent = cpu_time(rsa, plaintext, plaintext_len);
        float rate = cpu_cycles(rsa, plaintext,  plaintext_len);
        float Throughput = throughput(rsa, plaintext,  plaintext_len);
