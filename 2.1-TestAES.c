...
int aes128(unsigned char *plaintext, unsigned char *key, unsigned char *IV, size_t plaintext_len, size_t key_len, size_t IV_len)
{
...
{
    int byte_length_of_processed_data = plaintext_len;
    float time = cpu_time(aes128, plaintext, key, IV, plaintext_len, key_len, IV_len);
...
int main()
{
    ...
    FILE *fpt;
    fpt = fopen("Libtomcrypt256-CBC.csv", "w+");
    ...
    while (fgets(buffer, sizeof(buffer), ptr) != NULL) {
        ...
        float time_spent = 1000*cpu_time(aes128, plaintext, key, IV, plaintext_len, key_len, IV_len);
        float rate = cpu_cycles(aes128, plaintext, key, IV, plaintext_len, key_len, IV_len);
        float Throughput = throughput(aes128, plaintext, key, IV, plaintext_len, key_len, IV_len);
